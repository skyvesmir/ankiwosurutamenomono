...
## 9. 追記：ゲーム版の整備結果を取り込み（688 → 692件 / 2026-08-14）

GitHub 側のコミット `cc97fbe`「語源データをゲーム版の整備結果に差し替え」を取り込んだ記録。
...
| **合計** | **688** | **692** | **+4** |
| 例単語 | 3685 | 3705 | +20 |
| 学習カード（learnable） | 688 | 692 | +4 |

新規4件:

| ID | 見出しのコア意味 | テーマ |
|---|---|---|
| `Root-901` | 砦、陣営、囲って守る場所 | 関係・社会 ＞ 軍事・防衛 |
| `Root-902` | 閃く、輝く、雷が落ちる | 自然・物質 ＞ 光・天候 |
| `Root-903` | 影、日陰 | 自然・物質 ＞ 光・天候 |
| `Suf-901` | 〜であること、〜という性質 | 品詞化 ＞ 名詞化（性質・状態） |

例単語の和訳に「語源の理屈」の補足が **383件** 追記された（内容の改善）。

```
例: problem   「問題」          → 「問題（前に投げ出されたもの）」
    hyperbole 「誇張（法）…」   → 「誇張（法）、大げさな表現（度を越えて投げる）」
    otology   「耳学」          → 「耳科学」（誤訳の修正）
```

### 学習進捗への影響：なし

- 既存688件の ID は **全件保全**（消えたID 0件）
- `learnable` の変化 0件 / `importance` の変化 0件 / `themeGroup` の変化 0件
- 例単語の和訳が失われたもの **0件**（＝ §4 で作った195語の訳も全て生きている）
- 既存フィールドが空になったもの **0件**

### 【重要】取り込み時に見つかった表記の巻き戻り

ゲーム版の再生成で、§5-1 と §5-2 で直した表記規約が **再び巻き戻っていた**。

| 問題 | 取り込み直後 | 対処後 |
|---|---:|---:|
| `origin` のバッククォートが外れた | 536件 | 0件 |
| → 結果、学習ヒントに `productive` / `historical` が露出 | **539件** | **0件** |
| 言語名の「語」が落ちた（`ラテン` → `ラテン語`） | 253件 | 0件 |

`app-data.js` の `normEtym()` は `` `...` `` で囲まれた部分を消してヒントを作るため、
囲みが外れると内部メタ情報が学習者に見えてしまう（§5-1 と同じ問題の再発）。

→ 対処: `/home/user/etymwork/fix_origin_notation.py` を作成し、`origin` の**表記だけ**を補正。
他のフィールド（`core` / `examples` / `tips` / `theme` など）は 1文字も触っていないことを検証済み。

```
origin 以外が変わったエントリ: 0件
既存688件で「表示されるヒント」が v2 と変わったもの: 0件
```

なお、ゲーム版が「素の productive」を残したまま末尾に生産性列を重複追記していた
3件（`Pref-013` / `Pref-017` / `Pref-056`）は、v2 の origin 表記をそのまま復元した。

> **注意**: `古英語` は正しい表記なので「古英語語」にしてはいけない。
> 補正スクリプトの言語名リストからは除外している。
...
必須項目が欠けたカード:      0件
ヒントに生産性の漏れ:        0件
リンク解決できないカード:     0件（etym_links.json は v2 のまま・参照3714件すべて解決）
debugThemeCounts:        15テーマ・合計373（語根総数と一致）
meta.json の非語源キー:     words/phrases/words_full/phrases_full すべて不変
```

テストスイート: **9スイート 838 passed / 0 failed**
（`etym-v2.js` を件数692に更新し、この巻き戻りが再発したら落ちる検証を4項目追加）
